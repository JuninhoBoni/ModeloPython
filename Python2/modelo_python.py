...
#ADICIONAR PASTAS EXTERNAS - NECESSÁRIO NO PYTHON 2.4.
sys.path.append('%s/lib' % (diretorioPython))
sys.path.append('%s/files' % (diretorioPython))
...
def variaveisExternas():
    ...
    log.adicionar('''AS VARIÁVEIS USADAS FORAM: 
                    ...
                    etapa = %s, 
                    producao = %s, 
                    diretorioSaida = %s''' %(sys.argv[0], etapa, producao, diretorioSaida))
...
def exitPrograma(sysExit):
    ...
    if sysExit == 0:
        ...
        mensagem.append('FOI INFORMADO A VARIÁVEL etapa = %s, PORÉM ESTA ETAPA NÃO EXISTE NESTE PROGRAMA.' %(etapa))
        mensagem.append('---------  PROGRAMA FINALIZADO COM ERRO   ---------\n')
...
def queryXPTO(acao, txt, nome, query_principal):
    ...
    arquivoSaida = '%s%s'%(nome, hoje) 
    log.adicionar('GERANDO ARQUIVO: %s'%(arquivoSaida))
    
    if txt:
        arqSaida = open('%s%s.txt'%(diretorioLogs, arquivoSaida), 'a+')
        csv_txt = 'txt'
    else:
        arqSaida = open('/%s/%s.csv'%(diretorioSaida, arquivoSaida), 'a+')
    ...
    log.adicionar('GERANDO %s.%s'%(arquivoSaida, csv_txt))
    arqSaida.close()
    log.adicionar('%s.%s GERADO'%(arquivoSaida, csv_txt))
    return dados
...
def main():
    ...
    if etapa == '4' or etapa == 'all':
        ...
        log.adicionar('INICIO BLOCO_04 - PERÍODO %s À %s' %(dataInicial, dataFinal))
...
if __name__ == "__main__":
    ...
    logNome = ('%s.log' % (os.path.basename(__file__)))
